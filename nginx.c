...
int	zbx_module_nginx_status(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	*/
	...
	if (SUCCEED == zbx_tcp_connect(&s, CONFIG_SOURCE_IP, CONFIG_NGINX_HOST, (unsigned short)CONFIG_NGINX_PORT, 0))
	{
		/* for dev
		...
		*/
		...
		{
			net_error = 1;
			zabbix_log(LOG_LEVEL_WARNING, "module [nginx], func [zbx_module_nginx_status], get nginx status error: [%s]", zbx_tcp_strerror());
			SET_MSG_RESULT(result, zbx_dsprintf(NULL, "Get nginx status error [%s]", zbx_tcp_strerror()));
		}
